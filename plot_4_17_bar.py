...
for i in range(4):
    ...
    for j in range(3):
        ...
        data_plot.plot(kind='bar', stacked=False, ax=ax[i,j], legend=False)
        ax[i, j].legend(["Mpc"],loc='upper right')
        ax[i,j].set_xticks(np.arange(0,10,1),np.arange(900,6900,600),rotation=0)
